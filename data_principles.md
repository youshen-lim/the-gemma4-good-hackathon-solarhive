...
| Category | Count |
|---|---|
| (b) *should-call* (single + multi-tool chains + full 5-tool community audits across `get_weather` / `get_solar_production` / `get_battery_state` / `get_grid_status` / `get_nrel_pvwatts_baseline`) | **106** |
...
### 6.6 Tool-Result Message Format (training ↔ inference parity)

Every tool round in the corpus uses a TWO-message sequence per Gemma 4's
chat-template tool-handling protocol:

```python
{"role": "assistant", "tool_calls": [{"function": {"name": ..., "arguments": ...}}, ...]}
{"role": "tool",      "name": "<fn>", "content": "<JSON string>"}  # one per tool result
```

This format is generated by `solarhive_datagen.py` (line 2595–2599 builder
plus geo-variant examples at 4182–4220), validated by `solarhive_finetune.py`
preprocessing (line 802) and schema check (line 875), and emitted at
inference by `solarhive_inference.py` Cell 4 `generate_with_tools()`,
§13g `_ollama_agentic_loop()`, and `test_ollama_tools.py` Solution B
`build_gemma4_prompt()`. Inference-path message construction is
byte-identical to training-data construction — the model sees the exact
same multi-turn structure at inference that it was supervised on.

This 2-message form differs from Google's vanilla Gemma 4 function-calling
docs (which use a single message with both `tool_calls` and `tool_responses`
keys). We retain the 2-message form because that's what the v2 fine-tune
was trained on; switching would mismatch the training distribution.

### 6.7 Inference-time When2Call Validation

`solarhive_inference.py` Cell 11b runs three held-out probes per
`when2call_plan.md` Task W6, validating coverage of 3 of the 4 failure-mode
categories from the cited paper:

- **(b)** Well-specified in-scope query → expect correct tool call (e.g.,
  *"What's the current grid rate?"* → `get_grid_status`)
- **(c)** Under-specified query → expect follow-up question, not auto-filled
  defaults (e.g., *"How much will a 10 kW array produce today?"* should
  ask for location)
- **(d)** Out-of-scope query → expect refusal + redirect, not hallucinated
  tool (e.g., *"current air quality?"* should name the limit and point to
  airnow.gov)

Pre-W1+W2 (v1 model trained without `_UNABLE_TO_ANSWER` +
`_FOLLOW_UP_QUESTIONS` corpus categories): fails (c) + (d) by hallucinating
tools or auto-filling defaults — exactly the failure modes the paper
documents at 9–67% rates in untrained community models. Post-D5 (v2 model
trained with both categories): target 3/3 with zero regression on the
Run-6 8/8 baseline.
